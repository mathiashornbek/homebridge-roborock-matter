...
- 🛡️ **Verified by Homebridge.** Reviewed and endorsed by the Homebridge team. 1141 automated tests, zero known vulnerabilities, no analytics, and a startup designed to never crash your Homebridge — even when your Wi-Fi or the Roborock cloud has a bad day.
...
A counterexample arrived in [#9](https://github.com/mathiashornbek/homebridge-roborock-matter/issues/9) after the tests above: another bridge showing Apple Home a `WaterTankEmpty` fault, rendered properly, with a badge and a translated string. So "Apple never draws these" is too strong, and finding the condition it holds under is open work. It does not change the recommendation, because the sensor is not blocked either way: it needs no re-pairing and it does not depend on Apple rendering anything.
