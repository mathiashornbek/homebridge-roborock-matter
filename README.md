...
- 🛡️ **Verified by Homebridge.** Reviewed and endorsed by the Homebridge team. 1181 automated tests, zero known vulnerabilities, no analytics, and a startup designed to never crash your Homebridge — even when your Wi-Fi or the Roborock cloud has a bad day.
...
| `enableMatterTankFaultReporting`        | `true`       | Publish Matter's WaterTankEmpty fault, which Apple Home draws as a tap icon on the play button ([details](#why-the-robot-needs-attention))                                                                                              |
...
**Dock and tank conditions are not reported through Matter, and this is worth explaining.** The plugin can read them all accurately — empty clean-water tank, full waste-water tank, missing dust bag, blocked air duct — and two releases tried to surface them through Matter's fault attribute (`OperationalError`). 3 controlled tests on an S8 Pro Ultra with a genuinely empty clean-water tank showed it does not work: Apple Home drew no warning when the fault was published beside a Charging state, and drew no warning in the final test either, where the robot was raised all the way to the Matter **Error** state carrying "Clean water tank empty" — the tile simply kept reading Ready. **Apple Home drew nothing in any of those tests** — which is also why an earlier version removed the same write back in 1.4.61. The condition under which it _does_ draw one now has a likely explanation, and it is one this section previously got backwards: those tests ran on a **bridged** accessory. Homebridge registers every Matter accessory on one shared node, and 4 independent robot-vacuum projects — Matterbridge, 2 of Thoukydides' plugins and `matterbridge-roomba` — all report that Apple Home handles a bridged vacuum badly, and all ship a standalone-node mode to escape it. Nobody has published a controlled test of vacuum faults on a standalone node, so the honest statement is: Apple Home draws nothing from a bridge, measured 3 times on this hardware, and the standalone case is untested. The remaining dock conditions are therefore still not published.

**The empty clean-water tank is now the exception, and 3.12.0 publishes it.** A counterexample arrived in [#9](https://github.com/mathiashornbek/homebridge-roborock-matter/issues/9) after those tests: another bridge showing Apple Home a `WaterTankEmpty` fault, rendered properly, with a tap icon on the play button and a translated string. So "Apple never draws these" is too strong. The plugin now publishes Matter's `WaterTankEmpty` when the robot reports no clean water, and clears it when the tank is refilled. The robot is **not** forced into the Error state along with it — Wazza151's third test did exactly that and Apple still drew nothing, so it buys nothing measured and would cost a docked robot its Start command. No re-pairing is needed for this one: an error attribute is a live value, not a capability.

Whether your controller draws it is still Apple's call, and the condition that separated the two field results is not known. If your tile misbehaves, `enableMatterTankFaultReporting: false` in `config.json` puts it back.

The other route is unchanged and does not depend on Apple rendering anything: **the `Water Tank Empty` contact sensor** under [Home app sensors](#automations-in-apple-home) can carry a notification and drive an automation, which the tile icon cannot.
...
**`Water Tank Empty` is the one Apple Home has no other route to.** Matter's robot vacuum device type has no water-tank attribute of any kind, so no plugin can put the condition on the tile — this sensor is the whole of it. Point a Home notification at it and a mop run that would have been wasted becomes a message instead. It reports and nothing more: it never refuses to start a clean, because the plugin deciding your robot cannot mop on the strength of its own cached reading is a worse failure than a dry mop, and this project has shipped that bug before.
