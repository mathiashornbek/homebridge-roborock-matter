  ...
  async updateProduct(loginApi, productID, duid) {
    ...
    for (const array in list) {
      for (const product in list[array]["productList"]) {
        ...
        const productIDinPackage = this.findProductIDinPackage(
          productID,
          vacuum
        );
    ...
    const packageData = await loginApi.post(
      "api/v1/appplugin",
      appPluginRequest
    );
      ...
      try {
        ...
        if (packages[rr_package].version > currentVersion) {
          this.adapter.log.debug(
            `New version roborock package available: ${version}`
          );

          const response = await loginApi.get(zipUrl, {
            ...
            folder.forEach(async (relativePath, file) => {
              if (!file.dir) {
                ...
                if (fileContent) {
                  ...
                  const fileNameWithoutExtension = relativePath.slice(
                    0,
                  ...
                  this.adapter.setStateAsync(
                    `${objectPath}.${formattedNumber}`,
                    { val: fileContentBase64, ack: true }
                  );
                }
                i++;
              }
        ...
        this.adapter.catchError(
          err,
          "roborockPackageHelper.updateProduct",
          null,
          productID
        );
      }
